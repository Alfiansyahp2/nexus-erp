...
import { Card, Button, Typography, Row, Col, Statistic, message, Table, Space, Image } from 'antd';
...
const EmployeeDashboard = () => {
    ...
    const columns = [
        ...
        },
        {
            title: 'Check In',
            dataIndex: 'check_in', 
            sorter: (a, b) => { const vA = a['check_in'] ?? ''; const vB = b['check_in'] ?? ''; if (typeof vA === 'number' && typeof vB === 'number') return vA - vB; return String(vA).localeCompare(String(vB)); },
            key: 'check_in',
            render: (text, record) => (
        ...
        },
        {
            title: 'Check Out',
            dataIndex: 'check_out', 
            sorter: (a, b) => { const vA = a['check_out'] ?? ''; const vB = b['check_out'] ?? ''; if (typeof vA === 'number' && typeof vB === 'number') return vA - vB; return String(vA).localeCompare(String(vB)); },
            key: 'check_out',
            render: (text, record) => (
                <Space orientation="vertical" size="small">
                    <Text>{text ? new Date(text).toLocaleTimeString() : '-'}</Text>
